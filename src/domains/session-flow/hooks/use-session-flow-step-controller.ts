...
import { flowStepToChatMessage } from "../utils/step-to-chat-message";
import { useSessionFlowChatEngine } from "./use-session-flow-chat-engine";
...
export default function useFlowStepController({
  ...
  const { session, updateSession } = useSessionFlowState({ sessionId });

  const currentStepId = useMemo(() => session?.currentStepId || null, [session]);

  const previousStepIdRef = useRef<string | null>(null);

  const { addMessage, deleteMessage, messageExistsByStepId, clearMessages } = useSessionFlowChatEngine({ sessionId });
  ...
  const handleSystemAction = useCallback(
    ...
    [callbacks, clearMessages, session, sessionFlow.initialStepId, updateSession]
  );
  ...
  const handleSystemStep = useCallback(
    (step: StepOfType<"system">, messageId: string) => {
      ...
      setTimeout(() => deleteMessage(messageId), 3000);
    },
    [advanceToStep, handleSystemAction, deleteMessage]
  );

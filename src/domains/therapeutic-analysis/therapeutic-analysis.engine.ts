...
/**
 * Safely parses AI response into validated TherapeuticAnalysis
 */
...
/**
 * Builds analysis context prompt with session metadata and conversation history
 */
export function getAnalysisContextPrompt(
  userInput: string,
  ...
  return prevData.length === 0
    ...
    : ({
        ...
        content: JSON.stringify({
          current_message: userInput.trim(),
          ...(conversationContext && { recent_conversation: conversationContext }),
          previous_analyses: getAnalysisContext(prevData),
          ...(sessionContext && { session_context: sessionContext }),
          instructions: `
...
/**
 * Aggregates analysis context from previous analyses
 * @internal
 */
function getAnalysisContext(prevData: TherapeuticAnalysis[]): AnalysisContext {
